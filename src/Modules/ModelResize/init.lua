return function(RWE4,Plugin,EditorLibraries,Libraries,loader,RBLXGUI,EI)
    ...
    do
        ...
        Vars.DoJoints = Editor:GetModuleSetting(script.Name,"DoJoints",true)
	    Vars.DoUnions = Editor:GetModuleSetting(script.Name,"DoUnions",true)
	    Vars.MeshSmall = Editor:GetModuleSetting(script.Name,"MeshSmall",true)
	    Vars.AntiFreezeEnabled = Editor:GetModuleSetting(script.Name,"AntiFreezeEnabled",true)
		Vars.Preset = Editor:GetModuleSetting(script.Name,"Preset",'None')
		Vars.Percent = Editor:GetModuleSetting(script.Name,"Percent",100)
        API.AntiFreezeEnabled = Editor:GetModuleSetting(script.Name,"AntiFreezeEnabled",true)
        local function calculateChange(face, distance)
            local offset
            local movementOffset

            local min, max = getPartBounds(API.target)
        ...
        RWE4:AddConnection(Vars.Handles.MouseDrag:Connect(function(face, distance)
            ...
            if distance ~= 0 then
                ...
                local min, max = API.getPartBounds(Vars.target)
                local bounds = max - min
                ...
                if Vars.AntiFreezeEnabled then
                    ...
                    Vars.Resize(Vars.target, API.targetJoints, Vars.targetMeshes, false, (bounds + offset) / bounds, min, max)
        ...
        Editor:AddConnection(API.Handles.MouseButton1Down:connect(function()
            API.lastDragOffset = 0
            movementOffsetGlobal = Vector3.new()
    
            if API.AntiFreezeEnabled then
                API.HandlePart.Transparency = 0.5
                ...
                con = UIS.InputEnded:connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        ...
                        local min, max = API.getPartBounds(API.target)
                        local bounds = max - min
                        API.Resize(API.target, API.targetJoints, API.targetMeshes, false, API.HandlePart.Size / bounds, min, max)
    
                        for i, part in ipairs(API.target) do
                        ...
                        API.rethinkSelection()	
                        API.HandlePart.Transparency = 1
                        con:Disconnect()
                    end
                end)
            end
    ...
    local function getMesh(part) -- Gets the first mesh found in the part, or creates one if there is none.
    ...
    local function Resize(parts, joints, meshes, useProgress, percent, min, max)
		local key = tick()
    ...
    do
        ...
        RWE4:AddConnection(HandlesStepInput.Object.Changed:Connect(function(val)
            Vars.HandlesStep = tonumber(val)
			RWE4:SetModuleSetting(script.Name,"Step",API.HandlesStep)
        end))
        local DoJointsCheck = RBLXGUI.Labeled.new({
            Text = "Resize Joints?",
            Objects = gui.Checkbox.new({
                Value = Vars.DoJoints;
            })
        }, pageParamsBasic)

        RWE4:AddConnection(DoJointsCheck.Object.Changed:Connect(function(val)
            Vars.DoJoints = val
        ...
        RWE4:AddConnection(DoUnionsCheck.Object.Changed:Connect(function(val)
            Vars.DoUnions = val
        ...
        RWE4:AddConnection(MeshSmallCheck.Object.Changed:Connect(function(val)
            Vars.MeshSmall = val
        ...
        RWE4:AddConnection(AntiFreezeEnabledCheck.Object.Changed:Connect(function(val)
        ...
        RWE4:AddConnection(PercentBox.Object.Changed:Connect(function(val)
            Vars.Percent = tonumber(val)
			RWE4:SetModuleSetting(script.Name,"Percent",Vars.Percent)
        end))

        local PresetChoice = gui.Labeled.new({
        ...
        RWE4:AddConnection(PresetChoice.Object.Changed:Connect(function(index)
			if index == 1 then
				Vars.Preset = 'None'
			elseif index == 2 then
				Vars.Preset = 'GunMin'
            ...
            PercentBox.Object:SetDisabled(API.Preset ~= 'None')			
    ...
    local function getPartBounds(parts)
    ...
    local function rethinkSelection()
		...
		elseif API.Handles.Visible then
