...
local function YieldUntilReadyToFire(Player, ...)
	repeat until LoadedPlayers[Player] or not RunService.Heartbeat:Wait()
	FireClient(Player, ...)
end
...
function FireClient(Player, ...)
	...
	if Old then
		...
		RemoteService.send("Client",Player,"PseudoInstanceReplicatorClient", Old + 1, ...)
	else
...
if IsServer then
	Players.PlayerAdded:Connect(function(Player)
		if RemoteFunction:InvokeClient(Player) then -- Yield until player loads
			...
			for i = 1, NumReplicationOrder do
				...
				RemoteService.send("Client",Player,"PseudoInstanceReplicatorClient",i, self.__class.ClassName, Id, self.__rawdata)
			end
	...
	RemoteService.listen("Server","Send","PseudoInstanceReplicator",function(Player, ClassName, Id, Event, ...) -- Fire events on the Server after they are fired on the client
...
elseif IsClient then
	...
	RemoteService.listen("Client","Send","PseudoInstanceReplicatorClient",function(EventNumber, ClassName, Id, RawData, Assigned) -- Handle objects being replicated to clients
		repeat until OnClientEventNumber == EventNumber or not RunService.Heartbeat:Wait()
				...
				for Event in next, self.__class.Events do
					if Event ~= "Changed" then
						self[Event]:Connect(function(...)
							RemoteService.send("Server","PseudoInstanceReplicator",self.__class.ClassName, Id, Event, ...)
						end)
					end
				end
			end
