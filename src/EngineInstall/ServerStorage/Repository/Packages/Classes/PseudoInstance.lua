...
local function Metatable__index(this, i)
	...
	if Value == nil and not ClassTemplate.Properties[i] then
		...
		elseif ClassTemplate.Accessors[i] then
			return ClassTemplate.Accessors[i](self)			
...
local function Metatable__call(self, ...)
	local result = (Metatables[self] or self).__class.Call
	if result then
		result = result(self,...)
	end
	if result then
...
local DataTableNames = SortedArray.new{"Events", "Methods", "Properties", "Internals";"Accessors";};
...
function PseudoInstance.Register(_, ClassName, ClassData, Superclass)
	...
	local Accessors = ClassData.Accessors;
	
	for i = 1, #Accessors do
		Accessors[Accessors[i]] = function(obj) return false end;
		Accessors[i] = nil;
	end
