...
--[[		p1 = (offset * (i + d * z) + v0 * y) * decay + g
		...
		p1 = (offset*i + ((v0*y) + offset*((d*z)*f))*j/(f*c))*decay + g
		v1 = (v0 * (i * c) - (v0*(z*d) + offset*(z*f))*j) * (decay/c)
	else -- Overdamped
		local c = sqrt(d * d - 1)
...
function Spring:accelerate(a,dt)
    local p, v = self:Update(dt), self.f
    self.p, self.f = p, self.f + a
end
