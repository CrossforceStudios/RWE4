...
local CustomTypes = {
	...
	NumberRange = function(Value, Type)
		return Type == "NumberRange" and Value.Min and Value.Max
	end;

	PositiveNumberRange = function(Value, Type)
		return Type == "NumberRange" and Value.Min >= 0 and Value.Max >= 0 	
	end;
...
local function TransformTableCheckerData(PotentialTypes)
	...
	if not PotentialTypes[0] then -- It was already transformed if written to 0, no haxing pls
		...
		while i < AmountPotentialTypes do
			...
			if TypeCheck then
				Table.QuickRemove(PotentialTypes, i)
				i = i - 1
