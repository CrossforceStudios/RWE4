...
local CFrameInterpolator, lerpCF do
 CFrameInterpolator = {}
...
lerpCF  = function(c0, c1, alpha) -- (CFrame from, CFrame to) -> (float theta, (float fraction -> CFrame between))
	-- The expanded matrix
	local _, _, _, xx, yx, zx, 
	               xy, yy, zy, 
	               xz, yz, zz = components(inverse(c0)*c1)
...
lerpCF = function(c0, c1, t)
	return CFrameInterpolator.new(c0, c1)(t)
end;

local inverseLerp = function(min, max, num)
	local r = ((num - min) / (max - min))
	if math.abs(r) == math.huge then return 0 else return r end
end
return Table.Lock {
	number = Lerp;
	angle = angleLerp;
	inverseNumber = inverseLerp;
	Color3 = Color3Lerp;
	...
	ColorSequence = function(start, finish, alpha)
		...
		for i, ap in next, start.Keypoints do
			...
			if closestAbove == closestBelow then
				...
				bValue = Color3Lerp(closestAbove.Value , closestBelow.Value, p)

			end
			local interValue = Color3Lerp(bValue, ap.Value, alpha)
			local interp = ColorSequenceKeypoint.new(ap.Time, interValue)
		...
		for i, bp in next, finish.Keypoints do
			if not addedTimes[bp.Time] then
				...
				if closestAbove == closestBelow then
					...
					aValue = Color3Lerp(closestAbove.Value , closestBelow.Value, p)
				end
				local interValue = Color3Lerp(aValue, bp.Value, alpha)
				local interp = ColorSequenceKeypoint.new(bp.Time, interValue)
