...
Tween.new = Typer.AssignSignature(Typer.OptionalNumber, Typer.OptionalFunctionOrEnumerationOfTypeEasingFunction, Typer.OptionalString, Typer.OptionalBoolean, Typer.FunctionOrTableOrUserdata, Typer.Any,  function(Duration, EasingFunction, Tag, Override, Callback, Arg)
	...
	local OpenTween = OpenTweens[self.Tag]
...
function Tween.__index:Stop(Finished)
	if self.Running then
		...
		ObjectTable = OpenTweens[self.Tag]
		if ObjectTable then
			OpenTweens[self.Tag] = nil -- This is for override checks
		end
	end
...
function Tween.__index:Resume()
	if self.Duration == 0 then
		...
		if not self.Running then
			...
			ObjectTable = OpenTweens[self.Tag]
			if ObjectTable then
				OpenTweens[self.Tag] = self -- This is for override checks
			end
		end
	end
	return self
