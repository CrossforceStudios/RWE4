...
function Color.fromHex(Hex)
	...
	if Type == "string" then
		if string.byte(Hex) == Hash then Hex = Hex:sub(2) end -- Remove # from beginning
...
function Color.getRelativeLuminance(color)
...
Color.toDullerVariant = function(color,p)
	local hsv = {Color3.toHSV(color)};
	hsv[3] = hsv[3] * p
	return Color3.fromHSV(table.unpack(hsv))
end;
Color.GetRelativeLuminance = Color.getRelativeLuminance
...
function Color.textShouldBeBlack(color)
	return Color.getRelativeLuminance(color) > 0.179
end
