...
local RAIN_SOUND_ASSET = "rbxassetid://9116323146"
...
function Rain:Enable(tweenInfo)
	...
	if RunService:IsRunning() then -- don't need sound in studio preview, it won't work anyway
		SoundGroup.Parent = game:GetService("SoundService").SettingSounds.Game_FX
	end
...
function Rain:SetStraightSize(size)

	if typeof(size) ~= "NumberSequence" then
		error("bad argument #1 to 'SetStraightSize' (NumberSequence expected, got " .. typeof(size) .. ")", 2)
	end

	Emitter.RainStraight.Size = size

	for _,v in pairs(rainAttachments) do
		v.RainStraight.Size = size
	end
