...
function ObjectHighlight.new(Object, Color, HideIfVisible, MaxRenderDistance, Transparency, RenderObject)
	...
	if Object and RenderObject then
...
function ObjectHighlight.Enable(HighlightObject)
	...
	for OriginPart, HighlightPart in pairs(HighlightObject.Objects) do
		...
		HighlightObject.Janitor:Add(OriginPart.AncestryChanged:Connect(function(_,p)
				if not p then
					HighlightPart:Destroy()
				end
		end),"Disconnect",OriginPart)
	end
...
function ObjectHighlight.Destroy(HighlightObject)
	for OriginPart, HighlightPart in pairs(HighlightObject.Objects) do
		HighlightPart:Destroy()
		HighlightObject.Janitor:Remove(OriginPart)
		OriginPart.LocalTransparencyModifier = 1;
